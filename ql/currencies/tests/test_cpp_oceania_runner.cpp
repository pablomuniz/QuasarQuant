// C++ Runner to print QuantLib currency properties for Oceania currencies
#include <ql/quantlib.hpp> // Main QL header
#include <ql/currencies/oceania.hpp> // Oceania currencies
#include <iostream>
#include <string>
#include <vector>
#include <stdexcept>
...
void printCurrencyProperties(const std::string& code) {
    ...
    if (code == "AUD") currency = QuantLib::AUDCurrency();
    else if (code == "NZD") currency = QuantLib::NZDCurrency();
    else {
        std::cerr << "Error: Unknown currency code '" << code << "' in C++ Oceania runner." << std::endl;
        exit(1); // Exit with error code
    }

    // Debug print to stderr for specific currencies before normal output
    if (code == "AUD" || code == "NZD") { // Adjusted for Oceania codes
...
int main(int argc, char* argv[]) {
    ...
    if (!locale_set_name) {
        ...
        if (!locale_set_name) {
            ...
            locale_set_name = std::setlocale(LC_ALL, ""); // Environment default
            if (!locale_set_name) {
    ...
    if (locale_set_name) {
        ...
        std::cerr << "Error: All attempts to set C-style locale (en_US.UTF-8, C.UTF-8, \"\") failed." << std::endl;
    }
    ...
    try {
    ...
    } catch (const std::runtime_error& e_en_us) {
        ...
        try {
        ...
        } catch (const std::runtime_error& e_c_utf8) {
            ...
            } catch (const std::runtime_error& e_default) {
